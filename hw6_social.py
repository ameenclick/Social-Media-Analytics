...
def graphRegionComparison(regionDicts, title):
    ...
    for region in regionDicts:
        ...
        for feature in featureNames:
            if(feature not in regionDicts[region]):
                temp.append(0)
            else:
                temp.append(regionDicts[region][feature])
        regionFeatures.append(temp)
    sideBySideBarPlots(featureNames,regionNames,regionFeatures,title)
    return
...
def graphHashtagSentimentByFrequency(data):
    ...
    top50Common=mostCommonHashtags(hashtags,50)
    hashtagLst=[]
    freqLst=[]
    sentiLst=[]
    for hashtag in top50Common:
        hashtagLst.append(hashtag)
        freqLst.append(top50Common[hashtag])
        sentiLst.append(getHashtagSentiment(data,hashtag))
    scatterPlot(freqLst,sentiLst,hashtagLst,"Top 50 Hastag v/s Sentiment")
    return
